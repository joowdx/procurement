...
it('denies delete access to non-owner members', function () {
    $otherWorkspace = Workspace::factory()->create();

    // Add user as member with all permissions except ownership
    Membership::create([
        'workspace_id' => $otherWorkspace->id,
        'user_id' => $this->user->id,
        'role' => 'member',
        'permissions' => ['users' => true, 'files' => true, 'folders' => true, 'settings' => true],
        'joined_at' => now(),
    ]);

    // Even with all permissions, non-owner cannot delete workspace
    // This tests the service layer logic that controller uses
    expect($this->service->canAccessWorkspace($this->user, $otherWorkspace))->toBeTrue();
    expect($otherWorkspace->user_id !== $this->user->id)->toBeTrue(); // Verify not owner
});
